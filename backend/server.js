...
const bbRoutes = require("./src/routes/bbRoutes");
const contaPagamentoRoutes = require("./src/routes/contaPagamentoRoutes");
...
// `verify` guarda os bytes originais em req.rawBody -- necessário pra
// conferir a assinatura HMAC do webhook da Clicksign, que precisa do
// corpo exato recebido, não de uma versão reserializada do JSON.
app.use(
  express.json({
    limit: "25mb",
    verify: (req, res, buf) => {
...
app.use("/bb", bbRoutes);
app.use("/contas-pagamento", contaPagamentoRoutes);
...
app.use("/signatarios-fixos", signatarioFixoRoutes);
